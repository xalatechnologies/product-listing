...
- [ ] Verify `src/lib/auth/index.ts` exists and exports `authOptions`
- [ ] Verify NextAuth is configured with PrismaAdapter
- [ ] Verify EmailProvider is configured
- [ ] Verify session strategy is set to "database"
- [ ] Verify `NEXTAUTH_SECRET` is set in environment
- [ ] Verify `NEXTAUTH_URL` is set in environment
- [ ] Verify custom pages are configured: signIn, signOut, error, verifyRequest
...
- [ ] Test `/api/auth/signin` endpoint returns sign-in page
- [ ] Test `/api/auth/signout` endpoint works
- [ ] Verify session callback properly extends session with user data

### Task 2.3: Auth Pages Verification
- [ ] Verify `src/app/auth/signin/page.tsx` exists and renders sign-in form
- [ ] Verify `src/app/auth/signout/page.tsx` exists
- [ ] Verify `src/app/auth/error/page.tsx` exists and displays error messages
- [ ] Verify `src/app/auth/verify/page.tsx` exists for email verification
- [ ] Test sign-in flow: enter email → receive magic link → sign in
- [ ] Test sign-out flow: click sign out → redirected to home

### Task 2.4: Protected Route Middleware
- [ ] Create `src/middleware.ts` file
- [ ] Implement middleware to protect `/dashboard` and `/projects/*` routes
- [ ] Redirect unauthenticated users to `/auth/signin`
- [ ] Preserve intended destination after sign-in
- [ ] Test middleware: try accessing `/dashboard` while logged out → should redirect to sign-in
- [ ] Test middleware: access `/dashboard` while logged in → should allow access

### Task 2.5: Auth Utilities
- [ ] Verify `getServerAuthSession` function works in server components
- [ ] Verify `getServerAuthSession` function works in API routes
- [ ] Create client-side auth hook: `useSession` (if not using NextAuth's)
...
- [ ] Verify `src/lib/api/trpc.ts` exists
- [ ] Verify `createTRPCContext` includes session and database
- [ ] Verify `protectedProcedure` checks for authenticated user
- [ ] Verify `publicProcedure` allows unauthenticated access
- [ ] Test context creation in a test procedure
- [ ] Verify database client is accessible in procedures

### Task 3.2: tRPC Router Root
- [ ] Verify `src/lib/api/root.ts` exports `appRouter`
- [ ] Verify all routers are imported: project, brandKit, image, subscription, aPlus
- [ ] Verify all routers are added to `appRouter` object
- [ ] Verify `AppRouter` type is exported
- [ ] Verify `createCaller` function is exported
- [ ] Test that router compiles without errors

### Task 3.3: tRPC API Route Handler
- [ ] Verify `src/app/api/trpc/[trpc]/route.ts` exists
- [ ] Verify it handles GET and POST requests
- [ ] Verify it creates tRPC context with headers
- [ ] Test `/api/trpc/project.list` endpoint returns projects (when authenticated)
- [ ] Test error handling: unauthenticated request → returns 401

### Task 3.4: tRPC Client Setup (Server)
- [ ] Verify `src/lib/trpc/server.ts` exists
- [ ] Verify it exports `api` caller for server components
- [ ] Test using `api` in a server component
- [ ] Verify it uses `createCaller` with proper context

### Task 3.5: tRPC Client Setup (React)
- [ ] Verify `src/lib/trpc/react.tsx` exists
- [ ] Verify it exports `api` for React Query
- [ ] Verify it uses `@trpc/react-query` provider
- [ ] Verify `TRPCReactProvider` wraps the app in `src/app/layout.tsx`
- [ ] Test using `api.project.list.useQuery()` in a component
- [ ] Verify React Query devtools work (if enabled)

### Task 3.6: tRPC Error Handling
- [ ] Verify Zod validation errors are properly formatted
- [ ] Verify TRPCError instances return proper error codes
- [ ] Test error handling: invalid input → returns validation error
- [ ] Test error handling: unauthorized access → returns UNAUTHORIZED error
- [ ] Test error handling: not found → returns NOT_FOUND error
