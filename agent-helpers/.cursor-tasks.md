...
- [ ] Create Stripe account or use existing (REQUIRES USER ACTION: Go to stripe.com and create account)
- [ ] Create Product "Starter Plan" in Stripe Dashboard (REQUIRES USER ACTION: Create in Stripe Dashboard)
- [ ] Create recurring monthly Price for Starter Plan ($29/month) (REQUIRES USER ACTION: Create price in Stripe Dashboard)
- [ ] Copy Starter Plan price ID to `STRIPE_PRICE_STARTER` (REQUIRES USER ACTION: Copy from Stripe Dashboard to .env)
- [ ] Create Product "Professional Plan" in Stripe Dashboard (REQUIRES USER ACTION: Create in Stripe Dashboard)
- [ ] Create recurring monthly Price for Professional Plan ($79/month) (REQUIRES USER ACTION: Create price in Stripe Dashboard)
- [ ] Copy Professional Plan price ID to `STRIPE_PRICE_PROFESSIONAL` (REQUIRES USER ACTION: Copy from Stripe Dashboard to .env)
- [ ] Create Product "Agency Plan" in Stripe Dashboard (REQUIRES USER ACTION: Create in Stripe Dashboard)
- [ ] Create recurring monthly Price for Agency Plan ($299/month) (REQUIRES USER ACTION: Create price in Stripe Dashboard)
- [ ] Copy Agency Plan price ID to `STRIPE_PRICE_AGENCY` (REQUIRES USER ACTION: Copy from Stripe Dashboard to .env)
- [ ] Set up Stripe webhook endpoint (REQUIRES USER ACTION: Development: use Stripe CLI `stripe listen --forward-to localhost:3000/api/webhooks/stripe`, Production: add endpoint in Stripe Dashboard)
- [ ] Copy webhook signing secret to `STRIPE_WEBHOOK_SECRET` (REQUIRES USER ACTION: Copy from Stripe CLI or Dashboard to .env)
- [ ] Copy Stripe secret key to `STRIPE_SECRET_KEY` (REQUIRES USER ACTION: Copy from Stripe Dashboard to .env)
- [ ] Copy Stripe publishable key to `STRIPE_PUBLISHABLE_KEY` (REQUIRES USER ACTION: Copy from Stripe Dashboard to .env)
...
- [ ] TODO: Implement file deletion from Supabase Storage when project is deleted (noted in code)
...
- [ ] TODO: Implement file deletion from Supabase Storage (noted in code)
...
- [ ] TODO: Implement logo file deletion from Supabase Storage (noted in code)
...
- [ ] Verify function sends progress updates via Supabase Realtime or Inngest events (TODO: implement progress updates)
...
- [x] Verify `generate` procedure checks user has enough credits (TODO: implement credit check)
- [x] Verify `generate` procedure deducts credits before generation (TODO: implement credit deduction)
...
- [ ] Test generating without credits: should return error (requires credit system implementation)
...
- [x] Verify `generate` procedure verifies project belongs to user
- [x] Verify `generate` procedure checks user has enough credits (A+ costs more) (TODO: credit check commented, needs credit system)
...
- [ ] Create export history component or page (Future enhancement - requires database schema changes)
- [ ] Verify component displays past exports (Future enhancement)
- [ ] Verify component shows: platform, date, download link (Future enhancement)
- [ ] Verify download links use signed URLs (if exports are private) (Future enhancement)
- [ ] Test export history: should display past exports (Future enhancement)
...
- [x] Verify section shows export history for project (Future enhancement - export history not yet implemented)
...
- [ ] Use pagination for large lists (not implemented yet - future enhancement)
...
- [ ] Write E2E test for image generation flow (requires running app with AI services)
- [ ] Write E2E test for A+ content generation flow (requires running app with AI services)
- [ ] Run E2E tests: should all pass (requires running app)
...
- [ ] Verify RLS policies are enabled on all tables (requires Supabase setup)
- [ ] Verify storage policies restrict access appropriately (requires Supabase setup)
